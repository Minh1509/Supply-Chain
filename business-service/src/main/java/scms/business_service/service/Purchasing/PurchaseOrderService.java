...
public class PurchaseOrderService {
    ...
    private PurchaseOrderDto convertToDto(PurchaseOrder purchaseOrder, List<PurchaseOrderDetail> details) {
        ...
        CompletableFuture<CompanyDto> companyFuture = CompletableFuture.supplyAsync(
                () -> externalServicePublisher.getCompanyById(purchaseOrder.getCompanyId()), executor);

        CompletableFuture<CompanyDto> supplierFuture = CompletableFuture.supplyAsync(
                () -> externalServicePublisher.getCompanyById(purchaseOrder.getSupplierCompanyId()), executor);

        CompletableFuture<WarehouseDto> warehouseFuture = CompletableFuture.supplyAsync(
                () -> externalServicePublisher.getWarehouseById(purchaseOrder.getReceiveWarehouseId()), executor);
        ...
        CompanyDto supplier = supplierFuture.join();
        WarehouseDto warehouse = warehouseFuture.join();
        ...
        List<PurchaseOrderDetailDto> detailDtos = details.parallelStream()
                .map(detail -> {
                    PurchaseOrderDetailDto detailDto = new PurchaseOrderDetailDto();
                    detailDto.setPurchaseOrderDetailId(detail.getPurchaseOrderDetailId());
