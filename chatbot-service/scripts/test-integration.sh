...
STATS=$(docker stats --no-stream chatbot-service-prod --format "table {{.Container}}\t{{.CPUPerc}}\t{{.MemUsage}}")
echo "$STATS"
echo -e "${GREEN}✓ Resource usage checked${NC}"

# Test 12: Service Logs
echo -e "\n${YELLOW}Test 12: Service Logs (checking for errors)${NC}"
ERROR_COUNT=$(docker logs chatbot-service-prod --tail 100 2>&1 | grep -i "error" | grep -v "error_message" | wc -l)
if [ "$ERROR_COUNT" -eq 0 ]; then
    echo -e "${GREEN}✓ No errors in recent logs${NC}"
else
    echo -e "${YELLOW}⚠ Found $ERROR_COUNT error(s) in logs${NC}"
    echo "Recent errors:"
    docker logs chatbot-service-prod --tail 100 2>&1 | grep -i "error" | grep -v "error_message" | tail -3
fi
