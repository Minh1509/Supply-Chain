...
if [ "$ENV" = "prod" ]; then
    docker-compose -f $COMPOSE_FILE build chatbot-service
else
...
docker-compose -f $COMPOSE_FILE up -d chatbot-service
...
echo "  - Stop service: docker-compose -f $COMPOSE_FILE stop chatbot-service"
echo "  - Restart service: docker-compose -f $COMPOSE_FILE restart chatbot-service"
