...
docker-compose -f $COMPOSE_FILE stop chatbot-service
echo -e "${GREEN}✓ Services stopped${NC}"

# Remove containers
echo -e "\n${YELLOW}Removing containers...${NC}"
docker-compose -f $COMPOSE_FILE rm -f chatbot-service
...
if [ "$REMOVE_VOLUMES" = "--volumes" ]; then
    ...
    if [ "$confirm" = "yes" ]; then
        if [ "$ENV" = "prod" ]; then
            ...
            docker volume rm chatbot-service_ollama-data 2>/dev/null || true
        fi
