...
if docker ps | grep -q "chatbot-service-prod"; then
...
ERROR_COUNT=$(docker logs chatbot-service-prod --tail 50 2>&1 | grep -i "error" | wc -l)
if [ "$ERROR_COUNT" -eq 0 ]; then
    echo -e "${GREEN}✓ No errors found in recent logs${NC}"
else
    echo -e "${YELLOW}⚠ Found $ERROR_COUNT error(s) in recent logs${NC}"
    echo "Recent errors:"
    docker logs chatbot-service-prod --tail 50 2>&1 | grep -i "error" | tail -5
fi
