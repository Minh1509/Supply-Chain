...
export class LocalAIService {
  ...
  constructor(
    ...
    const baseUrl = openaiConfig.baseUrl || 'http://localhost:8080/v1';

    this.model = new ChatOpenAI({
      openAIApiKey: 'not-needed',
      modelName: openaiConfig.model || 'gpt-3.5-turbo',
      temperature: openaiConfig.temperature || 0.7,
      maxTokens: openaiConfig.maxTokens || 2048,
    ...
    this.logger.log(`Using LocalAI at ${baseUrl}`);
  }
  ...
  async generateResponse(
    ...
    try {
      const langchainMessages = this.buildMessages(messages, systemPrompt, additionalContext);
  ...
  async analyzeIntent(
    ...
    try {
      const prompt = this.promptService.getIntentRecognitionPrompt(message, conversationHistory);
